...
spudsbyyear_grouped <- spudsbyyear_grouped[!(spudsbyyear_grouped$acc_sum < .02),]
...
write_csv(tibble(countyid=avector),'analyticsample_countyids_prod.csv')
...
spudsbyyear %>% mutate(
  ...
  y = coalesce(y,0)
) %>% dplyr::select(STATE,COUNTY,spudyear,y) %>% write_csv('spudsbyyear_BOE.csv')
...
setwd(.rt_data)
write.csv(tmpmat,'tmpmat_BOE.csv')
...
for (w in c('weighted','unweighted')){
  ...
  png(paste0(w,'ClusterPlot_BOE.png'),width=2000,height=1200)
  ...
  for (i in 1:maxNclust){ #5
    ...
    if(w == 'unweighted') {
      poisson_mm <- stepFlexmix(tmpmat ~ tots, 
                                k = i,
                                model = FLXMCfepois(),
                                control = list(tolerance = 1e-15,iter.max = 1000000,minprior = 0),nrep = 1)
      
    } else if(w == 'weighted') {
      poisson_mm <- stepFlexmix(tmpmat ~ tots, 
                                k = i,
                                model = FLXMCfepois(),
                                control = list(tolerance = 1e-15,iter.max = 10000000,minprior = 0),
                                weights = as.integer(tots),nrep = 1)
    }
    ...
    setwd(.rt_data)
    write.csv(dtmp,paste0(w,i,'dtmp_BOE.csv'))
    setwd(.rt_output)
    ...
    tmp2$clust <- hashmapcluster_old2new[tmp2$clust] # rd added 2020-07-11: switch old clusters to new, this wasn't being done here before
    
    setwd(.rt_output)
    tmp2 %>% saveRDS(paste0('production',w,i,'fepoisson_clusters.rds'))
    setwd(.rt_data)
    write.csv(tmp2,paste0(w,i,'tmp2_BOE.csv'))
    setwd(.rt_output)
